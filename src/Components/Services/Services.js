...
const Services = () => {
    ...
    const [flippedId, setFlippedId] = useState(null); // Track flipped state for each card

    const handleFliped = (serviceId) => {
        setFlippedId((prev) => (prev === serviceId ? null : serviceId)); // Toggle flip state for the clicked card
    }
